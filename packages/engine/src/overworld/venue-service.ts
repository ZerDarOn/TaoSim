...
import type { Character, RealmType, VenueDef, VenueType } from '@taosim/contracts';
...
export class VenueService {
  ...
  static canEnter(player: Character, venueId: string): VenueEnterCheck {
    ...
    if (venue.requiredRealm) {
      const playerRealm = player.realm.split('_')[0] as RealmType;
      if (REALM_ORDER[playerRealm] < REALM_ORDER[venue.requiredRealm]) {
