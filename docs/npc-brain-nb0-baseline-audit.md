...
> 状态：NB2 shadow 与首轮长期校准已完成，待唯一提交边界后进入 NB2.3 单写
> 日期：2026-08-31
...
| 开关 | 阶段 | 默认 | 允许行为 |
|---|---|---|---|
| `npcBrainV2Mode` | NB2 | `legacy` / `shadow` / `single-write` | 旧链、只诊断或对完整覆盖目标单写；未覆盖目标安全回退 |
| `battleEngineV2` | NB4 | 先测试开，验收后正式开 | 决定 UI 适配器内核；切换不改变结果协议 |

`npcBrainV2Mode` 已进入首批安全单写；待所有目标节点完成迁移后删除旧选择入口与兼容字段 `npcBrainV2Shadow`。`battleEngineV2` 成为正式默认且存档/回放通过后，删除 v1 开关和旧引擎。开关不是长期架构。
...
#### NB1.3 完成记录（2026-08-31）

- 新增 `BrainState` v1 及确定性初始化器；旧 `MindState` 只投影为初始目标/行动，未获得第二个提交入口；
- 新增 v6→v7 存档迁移，并把当前存档版本收口为共享常量，活动 NPC 与 `archivedNpcs` 均可补齐 Brain；
- 关系与生平以有界集合初始化 Belief/Memory，身体状态只派生 Emotion，伤毒权威仍在 `WorldState.conditions`；
- 新增纯函数不变量检查，覆盖悬空 NPC/地点/资产、未知版本、越界认知/身体值、计划与行动错链；
- IndexedDB 真实往返覆盖目标、计划、记忆、错误信念和长期伤势；
- 848 具名 NPC 全量测试负载样本：单月约 18.7 ms、一年度约 264.3 ms、序列化世界状态约 3.05 MiB（宽松门禁 1 秒/10 秒/15 MiB）。

NB1 已完成。下一阶段只能先进入 NB2 shadow：新调度器只输出建议与诊断，不提交世界变化。
...
- `NpcBrainNodeRegistry` 已注册修炼、闭关、突破、云游四个有真实执行器的节点，并支持逐节点开关和重复注册防护；
- 影子调度器已实现硬前置、归一化效用分解、稳定个体噪声、冷却、迟滞和满意选择；
- 调度只读取单个 NPC、Brain 与其长期身体状态，不查询全知 WorldState，也不消费主世界 RNG；
- `WorldEngine` 在旧 Mind 选出行动后做只读对照，关闭与开启 shadow 的世界状态逐字段一致；
- 每月报告和会话累计报告均只保留聚合计数与最多 20 条分歧样本，不写存档、不逐 NPC 打日志；
- shadow 评估异常已与世界推进隔离：损坏 Brain 只增加有界 `evaluation_error` 诊断，不影响旧 Mind 单写结果；
- 820 NPC 全量测试负载样本约 47.5 ms/月；三种子 × 两年样本均约 2 万次评估，建议覆盖率 100%，首轮匹配率 68.5%-73.0%；
- 根据转移矩阵修复求名者低修为阶段错误云游后，三种子匹配率提升至 83.1%-84.7%；保留 `breakthrough→seclude`（旧 Mind 前置与执行器矛盾）和 `challenge→wander`（节点未覆盖）作为显式差异，不为追求匹配率复制错误；
- 当前全员月度 tick 中每人每月都到期，单独维护唤醒索引没有剪枝收益；该索引延后到 NB3 日/事件级调度，并在提高调度频率前作为硬门禁。
...
G0 已使仓库回到可信全绿基线，NB1、NB2 shadow 与首轮校准已完成。下一步先建立首批动作唯一提交边界，再按 NB2.3 单写 → NB3（含唤醒索引）→ NB4 → NB5 顺序逐阶段提交；每阶段通过质量门后再删除对应旧路径，避免把两套大脑和三套战斗永久留在项目里。
